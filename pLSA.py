...
class Document(object):
    ...
    def _clean_word(self, word):
        ...
        return WordNetLemmatizer().lemmatize(word)  if re.match(Document.WORD_REGEX, word) else None
...
class Corpus(object):
    ...
    def p_wt(self, w, T, number_of_topics):
        pw = 1 / len(self.vocabulary)
    ...
    def p_td(self, d, T, number_of_topics):

        proj_all =np.zeros(shape=(number_of_topics, number_of_topics))

        for i in range(number_of_topics):
            proj_all[i] = (proj(d, T[i]))
        return proj_all.dot(d).dot(d.T)
    ...
    def plsa(self, number_of_topics, max_iter):
        ...
        print('d t '+str(len(self.document_topic_prob)))
        print('d t 1 '+str(len(self.document_topic_prob[1])))
        ...
        self.document_topic_prob = (self.document_topic_prob/self.document_topic_prob.sum(axis=1)[:,None])
        ...
        self.topic_word_prob = (self.topic_word_prob/self.topic_word_prob.sum(axis=1)[:,None])
        ...
        for iteration in range(max_iter):
            ...
            for d_index, document in enumerate(self.documents):
                for w_index in range(vocabulary_size):
                    ...
                    if sum(prob) == 0.0:
                        print
                        "d_index = " + str(d_index) + ",  w_index = " + str(w_index)
                        print
                        "self.document_topic_prob[d_index, :] = " + str(self.document_topic_prob[d_index, :])
                        print
                        "self.topic_word_prob[:, w_index] = " + str(self.topic_word_prob[:, w_index])
                        print
                        "topic_prob[d_index][w_index] = " + str(prob)
                        exit(0)
                    else:
                        normalize(prob)
    ...
    def new_plsa(self, number_of_topics, max_iter):
        ...
        self.topic_prob = np.zeros([number_of_documents, len(self.vocabulary), number_of_topics],
                               dtype=np.float)  # P(z | d, w)
        self.topic_basis = np.eye(number_of_topics)
        print('d t '+str(len(self.document_topic_prob)))
        print('d t 1 '+str(len(self.document_topic_prob[1])))
        print(number_of_documents)
        print(len(self.vocabulary))
        ...
        for iteration in range(max_iter):
            ...
            # update P(w | z)
            for z in range(number_of_topics):
                w = self.p_wt(self.topic_word_prob[:, z], self.topic_basis, number_of_topics)
                for w_index in range(vocabulary_size):
                    s = 0
                    for d_index in range(len(self.documents)):
                        count = self.term_doc_matrix[d_index][w_index]
                        s = s + count * w#self.topic_prob[d_index, w_index, z]
                    self.topic_word_prob[z][w_index] = s
                normalize(self.topic_word_prob[z])
            ...
            for d_index in range(len(self.documents)):
                for z in range(number_of_topics):
                    s = 0
                    d = self.p_td(self.document_topic_prob[z], self.topic_basis, number_of_topics)
                    for w_index in range(vocabulary_size):
                        count = self.term_doc_matrix[d_index][w_index]
                        s = s + count * d# self.topic_prob[d_index, w_index, z]
                    self.document_topic_prob[d_index][z] = s
                #                print self.document_topic_prob[d_index]
                #                assert(sum(self.document_topic_prob[d_index]) != 0)
            print(self.document_topic_prob)
            self.document_topic_prob = (self.document_topic_prob / self.document_topic_prob.sum(axis=1)[:, None])
        self.topic_basis = rotate(self.topic_basis, 1)
                #normalize(self.document_topic_prob[d_index])